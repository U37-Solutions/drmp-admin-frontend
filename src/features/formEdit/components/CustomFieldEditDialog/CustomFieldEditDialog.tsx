...
const CustomFieldEditDialog = ({ open, onClose, mode, field }: Props) => {
  const isCustomFieldId = !!field?.id && field.id >= 0;
  const { data, isPending } = useQuery<CustomFieldDTO>({
    ...
    enabled: isCustomFieldId && mode === CustomFieldEditDialogMode.edit,
  });
  ...
  return (
    <Modal
      centered
      destroyOnHidden
      loading={isCustomFieldId && isPending}
      title={
    ...
    >
      {mode === CustomFieldEditDialogMode.editStatic ? (
