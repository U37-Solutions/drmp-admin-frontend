...
import type { StaticFieldSchema } from '@features/formEdit/validation.ts';

import styles from './OptionsBuilderField.module.scss';

const DictionaryOptionsBuilderField = ({
  control,
  errors,
}: {
  control: Control<StaticFieldSchema>;
  errors: FieldErrors<StaticFieldSchema>;
}) => {
        ...
        <div key={item.id}>
          ...
          <Flex vertical gap={2}>
            {errors.options?.[index]?.name?.message && (
              <span className="field-error">{errors.options[index].name.message}</span>
            )}
          </Flex>
        </div>
      ))}
...
export default DictionaryOptionsBuilderField;
